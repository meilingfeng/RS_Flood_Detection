...
bb_list<-bb_list[c("NJ","CT","NH","ME")]
```

#3. Set time intervals of interest
```{r}
#nest data was collected 2010-2021, but HLS only starts in 2013 with landsat 8, get the breeding seasons in these years
...
search_req[["NJ"]]<-search_req_list

search_req_list<-list()
  for(j in 25:length(times)){
search_body <- list(limit=300,
                    return=300,
                    datetime=times[j],
                    bbox= bb_list[["CT"]],
...
save(list=c("search_req","state_list","search_df"),file="Outputs/CT_NJ_NH_ME_bands_All_nest_data.Rdata")

save(list=c("search_req","state_list","search_df"),file="Outputs/CT_bands_All_nest_data.Rdata")
```